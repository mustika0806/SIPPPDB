...
class AdminSiswaBaruController extends Controller
{
    ...
    public function confirmation(Siswa $siswa)
    {
        try {
            ...
            if ($dokumen) {
                // 1. Ambil data kelas terkait dari siswa
                // Asumsi: model Siswa memiliki kolom 'kelas_id' atau relasi 'kelas'
                $kelas = Kelas::find($siswa->kelas_id);

                // 2. Validasi apakah kelas ditemukan dan kuota masih tersedia
                if (!$kelas) {
                ...
                // 3. Update status dokumen dan siswa
                ...
                // 4. Kurangi kuota kelas sebanyak 1
                $kelas->decrement('kuota_tersedia');
                ...
                return redirect()->back()->with('succes', 'Data siswa berhasil diubah dan kuota berkurang');
            } else {
